...
- [x] **測試基礎設施** — `mockgen` 產 `db/mock` 與 `cache/mock`（`emit_interface` 就是為了這個）+
      handler table-driven 測試，參考 simple_bank 的模式（見「測試」一節）
- [x] **Dockerfile + CI** — multi-stage build 產出極小執行 image（含 tzdata，cron 時區才正確），
      GitHub Actions 跑 build/vet/test + docker build（見「Docker 與 CI」一節）
- [x] **排程任務 + asynq 背景任務** — asynq 內建 scheduler 到點 enqueue、worker 執行，
      多 instance 用 `asynq.Unique` 去重；首個任務：每天凌晨清理超過 3 個月的 operation_logs
      （見「排程任務與背景任務」一節）
